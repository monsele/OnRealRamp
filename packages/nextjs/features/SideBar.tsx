...
export default function SideBar({ tab, setTab }: Props) {
  return (
    <div className="h-screen fixed  w-[20vw] bg-white flex flex-col justify-start">
      ...
      <div className="flex flex-col items-center gap-10">
        {navList.map(nav => (
          ...
          >
            {nav.title === "Settings" ? (
              <Setting2 size="24" color={nav.title === tab ? "white" : "black"} />
            ) : nav.title === "My Properties" ? (
              <Category size="24" color={nav.title === tab ? "white" : "black"} />
            ) : (
              <Home2 size="24" color={nav.title === tab ? "white" : "black"} />
            )}
            <p
              style={
                nav.title === tab
